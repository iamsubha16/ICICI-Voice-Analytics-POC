...
function generateNewCallId() {
    ...
    if (callIdInput) {
        const uid = "CALL_" + Math.random().toString(36).substring(2, 8).toUpperCase()
...
function renderCallHistory(calls) {
    ...
    calls.forEach((call, idx) => {
        ...
        const initials = (call.customer_id || "??").replace(/[^a-zA-Z0-9]/g, "").slice(0, 2).toUpperCase();
        ...
        card.innerHTML = `
            <div class="call-hist-left">
                ...
                <div class="call-hist-info">
                    ...
                    <div class="call-hist-meta">${call.call_id} · ${call.loan_type || "—"}</div>
                </div>
            </div>
...
function setFileInInput(file) {
    ...
    document.getElementById("dropzone-sublabel").textContent =
...
function updateStepperVisualization(progress) {
    ...
    if (progress >= 10 && progress < 45) {
        ...
        n1.className = "step-node completed"; c1.className = "step-connector completed";
        n2.className = "step-node active";
    } else if (progress >= 80 && progress < 100) {
        n1.className = "step-node completed"; c1.className = "step-connector completed";
        n2.className = "step-node completed"; c2.className = "step-connector completed";
        n3.className = "step-node active";
    } else if (progress === 100) {
        n1.className = "step-node completed"; c1.className = "step-connector completed";
        n2.className = "step-node completed"; c2.className = "step-connector completed";
        n3.className = "step-node completed"; c3.className = "step-connector completed";
...
function renderAuditReport(report) {
    ...
    // Loan type (e.g. MAX_CALL_DURATION placeholder)
    const loanBadge = document.createElement("span");
    loanBadge.className = "badge badge-muted";
    loanBadge.textContent = report.loan_type || "—";
    badgesEl.appendChild(loanBadge);

    // Meta fields (agent = phone/call_id, call_id = initiated by, loan_type = reference)
    ...
    document.getElementById("val-loan-type").textContent  = report.loan_type     || "—";
    ...
    document.getElementById("report-summary-text").textContent =
        intel.call_summary || "No summary available.";
    ...
    if (!obsContainer.innerHTML)
        obsContainer.innerHTML = "<li class='text-muted'>No special observations.</li>";
    ...
    if (!missedContainer.innerHTML)
        missedContainer.innerHTML = "<li>No significant missed opportunities flagged.</li>";

    // Next Action + Feedback
    document.getElementById("report-next-action").textContent =
        intel.recommended_next_action || "Standard follow-up procedure.";
    document.getElementById("report-overall-feedback").textContent =
        intel.overall_feedback || "Compliance audit complete.";
    ...
    document.getElementById("audit-mismatch-explanation").textContent =
        dispo.mismatch_explanation || "Disposition verified.";

    const conf = Math.round((dispo.confidence_score || 0.90) * 100);
    ...
    document.getElementById("gauge-flow").textContent = (scores.flow_correctness || 0) + "/10";
    document.getElementById("slider-flow-fill").style.width = flowVal + "%";

    const handVal = Math.round((scores.response_handling || 0) * 10);
    document.getElementById("gauge-handling").textContent = (scores.response_handling || 0) + "/10";
    document.getElementById("slider-handling-fill").style.width = handVal + "%";

    const qualVal = Math.round((scores.call_quality || 0) * 10);
    document.getElementById("gauge-quality").textContent = (scores.call_quality || 0) + "/10";
    ...
    if (!strengthsContainer.innerHTML)
        strengthsContainer.innerHTML = "<li>No strengths recorded.</li>";
    ...
    if (!improveContainer.innerHTML)
        improveContainer.innerHTML = "<li class='text-green'>Perfect compliance! No improvements needed.</li>";

    document.getElementById("coaching-alternative").textContent =
        coaching.suggested_alternative || "Coaching dialogues up to date.";
...
function renderTranscriptTurns(containerId, transcript, limit) {
    ...
    turns.forEach(turn => {
        ...
        const role    = speaker.toLowerCase() === "agent" ? "agent" : "customer";
